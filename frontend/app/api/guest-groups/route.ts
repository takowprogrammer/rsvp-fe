...
export async function GET(req: NextRequest) {
    ...
    try {
        const res = await fetch(getBackendEndpoint('/guest-groups'), {
            method: 'GET',
            headers: {
        ...
        if (!res.ok) {
            const result = await res.json().catch(() => ({ message: 'Failed to fetch guest groups' }));
            console.error('Guest groups API error:', result);
